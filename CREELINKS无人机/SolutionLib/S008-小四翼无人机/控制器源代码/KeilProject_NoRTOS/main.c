...
{
    ceTft180Op.initial(&ceTft180,Spi2,PB9GI,PC3AGI,PB11GIP);       //使用一路Spi，三路GPIO资源初始化TFT180显示模块

    ceLed1COp.initialByGpio(&ceLed1A,PC10GI);                       //使用一路GPIO资源来初始LED1A
    ceLed1COp.initialByGpio(&ceLed1B,PC11GI);                       //使用一路GPIO资源来初始LED1B
    ceLed1COp.initialByGpio(&ceLed1C,PC6GIP);                       //使用一路GPIO资源来初始LED1C
    ceLed1COp.initialByGpio(&ceLed1D,PC7GIP);                       //使用一路GPIO资源来初始LED1D
    ...
    ceBeepNSrcOp.initialByGpio(&ceBeepNSrc,PC12GI);                //使用一路Gpio资源口初始化无源蜂鸣器
    ...
    ceJoystickOp.initial(&ceJoystickRight,PC5AGI,PC4AGI,PD2CGI);    //使用两路Ad和一路Gpio初始化右侧摇杆模块

    ceDebugOp.printf("Initial Btn...\n");
    ceBtnx1Op.initialByGpio(&ceBtnS2A,PB6GIP,CE_NULL);              //使用一路Gpio口初始化单个按钮模块S2A，不用注册回调函数
    ceBtnx1Op.initialByGpio(&ceBtnS2B,PB8GIP,CE_NULL);              //使用一路Gpio口初始化单个按钮模块S2B，不用注册回调函数
    ceBtnx1Op.initialByGpio(&ceBtnS2C,PA12CGI,btnS2cCallBack);       //使用一路Gpio口初始化单个按钮模块S2C，注册回调函数以控制TFT屏的显示
    ceBtnx1Op.initialByGpio(&ceBtnS2D,PA11GIP,CE_NULL);              //使用一路Gpio口初始化单个按钮模块S2D，不用注册回调函数
