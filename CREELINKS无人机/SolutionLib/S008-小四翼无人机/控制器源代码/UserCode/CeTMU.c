...
CE_STATUS ceTMU_initialByWifi(void (*recvCallBack)(uint8* recvBuf, uint16 recvCount))
{
    ...
    ceESP8266Op.initial(&(ceTMU.ceESP8266),Uart2);
    ceDebugOp.printf("Set wifi work mode...\n");
    while(CE_STATUS_SUCCESS != ceESP8266Op.setWorkMode(&(ceTMU.ceESP8266),CE_ESP8266_MODE_AP))
    {
    ...
    while(CE_STATUS_SUCCESS != ceESP8266Op.createWifi(&(ceTMU.ceESP8266),CE_TMU_WIFI_SERVER_IP, CE_TMU_WIFI_SSID,CE_TMU_WIFI_PWD,1,CE_ESP8266_ECN_WPA2_PSK))
    {
    ...
    while(CE_STATUS_SUCCESS != ceESP8266Op.createServer(&(ceTMU.ceESP8266),CE_TMU_WIFI_SERVER_PORT,CE_ESP8266_SOCKET_MODE_TCP,ceTMU_recvCallBackByWifi))
    {
...
CE_STATUS ceTMU_initialByNrf(void (*recvCallBack)(uint8* recvBuf, uint16 recvCount), void (*sendCallBack)(void))
{
    ...
    ceNRF24L01POp.initial(&(ceTMU.ceNRF24L01P),Spi1,PB0AGIP,PB1AGIP);
        
        ceTMU.sendCallBack ();
    ceNRF24L01POp.recv(&(ceTMU.ceNRF24L01P),0,nrfAddress,ceTMU_recvCallBack);//启动接收
        return CE_STATUS_SUCCESS;
}
...
CE_STATUS ceTMU_sendData(uint8* dataBuf, uint16 dataCount)
{
    ...
    {
        ceESP8266Op.sendData(&(ceTMU.ceESP8266),0,dataBuf,dataCount);
    }else if(ceTMU.useType == CE_TMU_USE_NRF)//如果处于2.4G无线工作方式
    {
        ceNRF24L01POp.send(&(ceTMU.ceNRF24L01P),nrfAddress,dataBuf,dataCount);//发送数据给控制器
        ceNRF24L01POp.recv(&(ceTMU.ceNRF24L01P),0,nrfAddress,ceTMU_recvCallBack);//启动接收
