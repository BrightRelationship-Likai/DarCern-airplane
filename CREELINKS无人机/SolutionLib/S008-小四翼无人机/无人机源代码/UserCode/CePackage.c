...
/**
  ...
  */
uint8*  cePackage_dealSend(void)
{
    int i,j;
    int32* temp = (int32*)(&cePackageSend);
    uint8 sum=0;
    uint16 packCount = 0;
   // if((cePackageRecv.status & CE_CTL_TYPE_STATION)  != 0)//如果是地面站点控制方式，则发送全部数据；其它控制方式则只发送一包数据即可。
...
/**
  ...
  */
CE_STATUS    cePackage_dealRecv(uint8* recvBuf, uint16 recvCount)
{
    int i=0,j;
    int packIndex = 0;
...
/**
  ...
  */
const CePackageOp cePackageOp = {cePackage_initial,cePackage_dealSend,cePackage_dealRecv};
