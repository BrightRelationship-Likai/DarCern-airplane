...
#define CE_SLIDE_FILTER_SIZE    32//滑动平均滤波数组最大长度
/**
...
#define CE_FILTER_BASE_DEEP     40 /*!< 滤波器深度,需要根据自己的程序运行周期dtS确定，满足 CE_FILTER_BASE_DEEP*dtS < 0.2S即可*/
/*
 * @brief  毛刺滤波器属性
 */
typedef struct
{
    ...
    fp32 coes[CE_FILTER_BASE_DEEP]; /*!< 权值数组*/
    uint16 index;                 
    uint16 isUp;                   
}CeFilterBase;
...
typedef struct
{
    uint32          ceFilterNowType;    /*!< 当前无人机姿态解算及滤波方式*/
    uint32          ceFilterNewType;    /*!< 当前无人机姿态解算及滤波方式*/
    ...
    CeStatus        ceStatusZero;       /*!< 用于校正姿态零点，以消除机械误差*/
		
		fp32						pitchByAcc;         /*!< 由无人机加速度直接解算出来的pitch角*/
		fp32 						rollByAcc;          /*!< 由无人机角速度直接解算出来的roll角*/
}CeFilter;
/*
 *CeFilter操作对像
 */
typedef struct
{
    void (*initial)(void);/*!<                                               @brief  滤波初始化
                                                                             @param  cePackageSend:数据打包并发送使用的结构体
                                                                             @param  cePackageRecv:数据拆包并解析使用的结构体*/

    void (*filter)(CeStatus* ceNowStatus, fp32 dtS); /*!<                    @brief  输入当前角速度及加速度来计算当前无人机的姿态角，详细可参考CREELINKS相关文档
                                                                             @param  nowAcc:当前无人机加速度数据
                                                                             @param  ceNowGyr:当前无人机角速度数据
                                                                             @param  ceNowStatus:当前无人机姿态数据，四元数姿态角计算完毕后会修改此指针中的内容
